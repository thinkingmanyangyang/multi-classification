...
def load_vocab(dict_path, encoding='utf-8', simplified=False, startswith=None, real_words=None):
    """从bert的词典文件中读取词典
    """
    token_dict = {}
...
class TokenizerBase(object):
    ...
    def encode_plus(self, first_text, second_text=None, \
                    prefix_words=['期待', '仇恨', '悲伤', '惊奇', '焦虑', '欢乐', '爱', '愤怒'],
                    maxlen=None):
        # 这里暂时不做两句的处理，之后补充
        prefix_tokens = [self._token_start] + prefix_words + [self._token_end]

        if maxlen:
            maxlen = maxlen-len(prefix_words)-3

        idx = int(bool(self._token_start))
        first_tokens = self.tokenize(first_text, maxlen=maxlen)[idx:]
        prefix_token_ids = self.tokens_to_ids(prefix_tokens)
        prefix_segment_ids = [0] * len(prefix_token_ids)
        first_token_ids = self.tokens_to_ids(first_tokens)
        first_segment_ids = [1] * len(first_token_ids)

        token_ids = prefix_token_ids + first_token_ids
        segment_ids = prefix_segment_ids + first_segment_ids
        # print(token_ids)
