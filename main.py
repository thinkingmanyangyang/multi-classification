...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_batch_size", default=4, type=int,
                        help="训练集的batch_size")
    parser.add_argument("--eval_batch_size", default=126, type=int,
                        help="验证集的batch_size")
    ...
    train_dataset = convert_to_dataset(train_examples, tokenizer, max_length=args.max_seq_length, label_list=LABEL_LIST)
    dev_dataset = convert_to_dataset(dev_examples, tokenizer, max_length=args.max_seq_length, label_list=LABEL_LIST)
    ...
    if args.do_train:
        ...
        trains(args, train_dataset, dev_dataset, model, fold_num=None)
    ...
    if args.do_test or args.do_train:
        ...
        model = CombinationModel.load_model(args, config, args.output_dir)
        model = model.to(args.device)

        test(args, model, test_dataset=dev_dataset)
        oof_train = _predict(args, model, dev_dataset)
